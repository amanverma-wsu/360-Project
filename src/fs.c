/*
 * CougFS: Filesystem Init / Mount Implementation
 *
 * Handles formatting a new filesystem and mounting an existing one.
 */
...
int fs_mount(const char *disk_path)
{
    ...
    /* Initialize journal and check for recovery */
    journal_init();
    ...
    /* Mark as dirty (will be set clean on unmount) */
