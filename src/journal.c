/*
 * CougFS: Journaling / Crash Recovery Implementation
 *
 * Simple write-ahead journal. Before committing metadata changes,
 * the new block contents are written to the journal area. On commit,
 * the actual blocks are updated. On recovery, committed but
 * unapplied transactions are replayed.
 *
 * Journal layout (JOURNAL_BLOCKS blocks starting at JOURNAL_START):
 *   Block 0: journal_header_t
 *   Blocks 1+: journal entries and data
 */
...
/* Simple transaction log: we buffer entries in memory */
...
int journal_commit(uint32_t txn_id)
{
    ...
    /*
     * Write journal entries to the journal area on disk.
     * We write entry metadata + data blocks sequentially after the header.
     * For simplicity, we use blocks 1..N of the journal area.
     */
    ...
    for (int i = 0; i < active_txn.count; i++) {
        ...
        /* Write a combined entry: first 16 bytes are the entry header,
         * rest is block data (we sacrifice 16 bytes of block data for simplicity) */
    ...
    /* Now apply the changes to the actual blocks */
    for (int i = 0; i < active_txn.count; i++) {
...
int journal_recover(void)
{
    ...
    /* Read journal blocks looking for a committed transaction */
    int found_commit = 0;
    int entry_count = 0;
    uint32_t committed_seq = 0;

    /* First pass: check if there's a committed transaction */
    for (uint32_t b = JOURNAL_START + 1; b < JOURNAL_START + JOURNAL_BLOCKS; b++) {
    ...
    for (uint32_t b = JOURNAL_START + 1; b < JOURNAL_START + JOURNAL_BLOCKS; b++) {
        ...
        if (entry->op == JOURNAL_OP_WRITE && entry->seq == committed_seq) {
            /* Reconstruct the full block data */
