/*
 * CougFS: Main Entry Point
 *
 * Usage:
 *   cougfs format <disk.img>     - Format a new filesystem
 *   cougfs mount <disk.img>      - Mount and open interactive shell
 *   cougfs fuse <disk.img> <mountpoint> [fuse-options]  - Mount via FUSE
 */
...
static void usage(const char *prog)
{
    ...
    printf("\nExamples:\n");
    printf("  %s format disk.img\n", prog);
    printf("  %s mount  disk.img\n", prog);
}
...
int main(int argc, char *argv[])
{
    ...
    if (strcmp(command, "format") == 0) {
        ...
        /* Pass remaining args to FUSE (skip program name, "fuse", and disk path) */
        int fuse_argc = argc - 2;
        char **fuse_argv = argv + 2;
        /* Replace disk path arg with program name for FUSE */
        fuse_argv[0] = argv[0];
