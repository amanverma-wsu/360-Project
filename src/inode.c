/*
 * CougFS: Inode Operations Implementation
 *
 * Reads and writes inodes from/to the inode table on disk.
 * Manages block pointers (direct + single indirect).
 */
...
uint32_t inode_get_block(cougfs_inode_t *inode, uint32_t logical_block, int alloc)
{
    ...
    if (inode->indirect == INVALID_BLOCK) {
        ...
        if (iblk < 0)
            return INVALID_BLOCK;
        /* Initialize indirect block with INVALID_BLOCK */
