/*
 * CougFS: Concurrency Layer Implementation
 *
 * Provides a global filesystem rwlock and per-inode rwlocks
 * using POSIX pthread reader/writer locks.
 */
...
{
    if (ino < MAX_INODES)
        pthread_rwlock_rdlock(&inode_locks[ino]);
}

void inode_read_unlock(uint32_t ino)
{
    if (ino < MAX_INODES)
        pthread_rwlock_unlock(&inode_locks[ino]);
}

void inode_write_lock(uint32_t ino)
{
    if (ino < MAX_INODES)
        pthread_rwlock_wrlock(&inode_locks[ino]);
}
