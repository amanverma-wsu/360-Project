/*
 * CougFS: Disk I/O Layer Implementation
 *
 * Uses POSIX system calls (open, read, write, lseek, close, fsync)
 * to perform block-level operations on the virtual disk image.
 */
...
int disk_read_block(uint32_t block_num, void *buf)
{
    ...
    while (total < BLOCK_SIZE) {
        ...
        if (n == 0) {
            /* Unexpected EOF — zero-fill the rest */
