/*
 * CougFS: File Operations Implementation
 *
 * Implements create, open, read, write, seek, close, delete, truncate.
 */
...
int file_seek(int fd, int32_t offset, int whence)
{
    ...
    case 1: /* SEEK_CUR */
        ...
        break;
    case 2: /* SEEK_END */
        new_offset = (int32_t)inode.size + offset;
        break;
    default:
        return -1;
    }
...
int file_delete(uint32_t dir_ino, const char *name)
{
    ...
    if (inode.mode & COUGFS_S_IFDIR)
        return -1;  /* use dir_remove for directories */

    /* Close any open file descriptors for this inode */
    for (int i = 0; i < MAX_OPEN_FILES; i++) {
