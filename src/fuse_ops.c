/*
 * CougFS: FUSE Integration Implementation
 *
 * Maps FUSE callbacks to CougFS filesystem operations.
 * Requires libfuse-dev to compile. When FUSE is not available,
 * this module compiles to a stub.
 */
                          ...
                          off_t offset, struct fuse_file_info *fi)
{
    ...
    ctx_data.buf = buf;
    ctx_data.filler = filler;

    /* We use dir_list with a simple callback */
...
static int cougfs_mkdir(const char *path, mode_t mode)
{
    ...
    strncpy(dirname, last_slash + 1, MAX_NAME_LEN);
    dirname[MAX_NAME_LEN] = '\0';
